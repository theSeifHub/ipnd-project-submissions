...
modes = ['sd', 'fn']
players = ['default', 'human', 'random', 'reflect', 'cycle']
...
class HumanPlayer(Player):
    def move(self):
        ...
        while move not in moves:
            move = input("Rock, paper, scissors, spock or lizard? > ").lower()
            if move == 'quit':
                print("Bye!")
                raise SystemExit()
        return move
...
class Game:
    ...
    def play_game(self):
        ...
        print("Choose mode:\nWin by score difference(sd) OR "
              + "After fixed number of rounds(fn)")
...
if __name__ == '__main__':
    ...
    while opponent not in players:
        opponent = input(f"Type 'quit' anytime to exit game.\nChoose your opponent from {players}:\n").lower()
        if opponent == 'quit':
            print("Bye!")
            raise SystemExit()
...
# game = rpssl_code_expanded_version.Game(rpssl_code_expanded_version.__Player(), rpssl_code_expanded_version.__Player())
# game.play_game()